...
| 1 | **CLI 확인·설치** | `which graphify` 결과 없으면 사용자에게 *글로벌* vs *`.venv` 격리* 옵션 안내. default 글로벌 (`pip install graphifyy`). `.venv` 모드는 Decisions §설치 위치 결정 참조. 설치 후 `graphify --help` (또는 `.venv/bin/graphify --help`) 로 검증 |
...
| 사용자 발화 또는 환경 | 위치 | 명령 | 이후 명령 prefix |
|---|---|---|---|
| 명시 없음 (default) | **글로벌** | `pip install graphifyy` | 없음 (`graphify ...`) |
| ".venv 로", "프로젝트 격리", "워크스페이스별 환경", "venv 안에" | **`.venv`** | `python3 -m venv .venv && .venv/bin/pip install --upgrade pip && .venv/bin/pip install graphifyy` | `.venv/bin/` (또는 `source .venv/bin/activate` 후 prefix 생략) |
| 글로벌 이미 있고 `.venv` 추가 요청 | **양쪽** | 위 `.venv` 명령 추가 실행 | `.venv/bin/` 사용. INSTALL §1.4 "글로벌 + .venv 동시 설치 시 — skill 버전 동기화" 정책 따름 |

`.venv` 모드 선택 시 *모든 후속 단계*에서 `.venv/bin/graphify` 사용 (또는 활성화). always-on hook (Step 5) 도 `.venv/bin/graphify claude install`. 상세 → INSTALL §1.4.
...
부트스트랩 끝나도 *결과를 안 보면 절반의 가치만 회수*. 다음 5분 체크리스트:
...
# 또는: head -100 graphify-out/GRAPH_REPORT.md
...
GRAPH_REPORT.md 읽기 우선 순위:

1. **God Nodes top 5~10** — 코어 추상. 예상과 일치하면 ✅, 의외면 *리팩토링 후보*
2. **Communities — cohesion < 0.1 약한 영역** — 분할 가능 = 정리 후보
3. **Surprising Connections** — 놓쳤던 연결 (INFERRED 는 검토 후 채택 결정)
4. **Suggested Questions** — graph 가 *직접 추천하는* 질문. 그대로 AI에 던짐

이후 일상 사용:
- 코드 변경 → `graphify update .` (또는 `.venv/bin/graphify update .`) — AST-only, 무비용, 5~20초
- `graphify hook install` 한 경우 매 commit 자동 incremental
- 일상 운영 빈도·결정 → USAGE.md §2 / §4 참조
